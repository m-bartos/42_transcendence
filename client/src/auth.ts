...
export async function login(username: string, password: string): Promise<boolean> {
    try {
        ...
        const response = await fetch(`${getApiBaseUrl()}/auth/login`, requestOptions);
        console.log("hned za fetch login: ", response); 
...
export async function register(username: string, email: string, password: string): Promise<boolean> {
    try {
        ...
        // Odeslání požadavku na server - použití dynamické URL
        const response = await fetch(`${getApiBaseUrl()}/auth/user`, requestOptions);
        const data = await response.json();
...
export async function logout(): Promise<void> {
    ...
    if(checkAuth()){
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/auth/logout`, requestOptions);
            if (response.status === 200) {
                localStorage.removeItem('jwt_token');
                localStorage.removeItem('user');
                localStorage.clear();
...
export async function logOutFromAllSessions(): Promise<void> {
    ...
    if(checkAuth()){
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/auth/sessions/logout/all`, requestOptions);
            console.log("Response logOutFromAllSessions: ", response);
            if (response.status === 200) {
                await showAlert({
                    title: "Important message",
...
export async function refreshToken() : Promise<boolean> {
    if(checkAuth()) {
        ...
        try {
            const response = await fetch(`${getApiBaseUrl()}/auth/user/refresh`, requestOptions);
            const data = await response.json();
            if (response.ok) {
                localStorage.setItem('jwt_token', data.data.token);
                console.log("Refresh token successssssssssssssssssssssssssssssssssssssssssssssssss");
