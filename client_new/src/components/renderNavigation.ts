...
export function renderNav(parentElement: HTMLElement): void {
    ...
    nav.id = 'mainNavBar'
    nav.className = ' p-4  md:pt-0 lg:p-4 relative w-full border-b border-black/20';
    nav.innerHTML = `
        ...
        <div id="mobileMenu" class="transform scale-y-0 max-h-0 opacity-0 transition-all duration-500 ease-in-out origin-top absolute top-full left-0 w-full bg-gray-600/95 flex-col md:hidden rounded-b-lg z-50 overflow-hidden text-center">
            <a href="/" data-navigo class="block text-white  opacity-75 hover:opacity-100  text-3xl font-normal px-4 py-6  border-y border-gray-500">Play NOW</a>
            ...
            <a href="/profile" data-navigo class="block text-white opacity-75 hover:opacity-100  text-3xl font-normal px-4 py-6 border-b border-gray-500">Profile</a>
