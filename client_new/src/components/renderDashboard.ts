...
async function getPlayerData(originalPlayerId?: number): Promise<PlayerData | null> {
    if (!originalPlayerId) {
...
function createMainContainer(): HTMLDivElement {
...
function renderPlayerHeader(container: HTMLElement, player: PlayerData): void {
    ...
    const header = createElement('h2', 'text-4xl font-normal text-center mb-6', `- ${player.username || 'Unknown Player'} -`);
...
function renderAdditionalStats(
    ...
    const opponentsText = createElement(
        ...
        `- has played against ${playerStats.uniqueOpponents} unique opponents -`
    );
    ...
    const opponentsScoreText = createElement(
        'p',
        'text-lg font-normal text-center mb-6',
        `- while his opponents scored ${playerStats.totalEnemyScore} points -`
    );
